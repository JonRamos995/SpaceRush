...
namespace SpaceRush.UI
{
    public class WorkshopSlotUI : MonoBehaviour
    {
        ...
        public void UpdateUI()
        {
            ...
            if (StartButton != null) StartButton.interactable = !isBusy && hasRecipes;
            if (RecipeDropdown != null) RecipeDropdown.interactable = !isBusy && !slot.IsAutomated && hasRecipes; // Lock dropdown if Auto? Or allow changing?

            // If Automated, maybe lock recipe change while working?
            // Manager logic: "if (slot.IsAutomated && !string.IsNullOrEmpty(slot.ActiveRecipeID)) StartJob..."
            // So if user changes recipe in dropdown, we should probably update ActiveRecipeID immediately?
            // Currently StartJob sets ActiveRecipeID.
            // If Auto is on, we need a way to set the "Target Recipe" without starting a manual job immediately if it's busy.
            // But `StartJob` checks `if (slot.IsWorking) return;`.
            // So user can only switch recipe when idle.
            ...
            {
                if (InstallAIButton != null) InstallAIButton.gameObject.SetActive(true);
